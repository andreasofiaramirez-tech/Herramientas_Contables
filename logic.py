# ==============================================================================
# 1. CONFIGURACIÓN GLOBAL Y CONSTANTES DE TOLERANCIA
# ==============================================================================
...
TOLERANCIA_MAX_USD = 0.50     # Margen permitido en Dólares
TOLERANCIA_CERO = 0.00        # Para cruces que deben ser exactos
...
# ==============================================================================
# 3. MÓDULOS DE CONCILIACIÓN - GRUPO MAYOREO
# ==============================================================================

# --- (A) Módulo: Fondos en Tránsito (BS) (111.04.1001)---
...
# --- (D) Módulo: Deudores Empleados - Otros (ME) (114.02.6006) ---
...
# --- (E) Módulo: Cuentas de Viajes (114.03.1002) ---
...
# --- (F) Módulo: Otras Cuentas por Pagar (VES) (212.05.1019) ---
...
# --- (H) Módulo: Asientos por Clasificar (VES) (212.05.1005) ---
...
# --- (I) Módulo: Devoluciones a Proveedores (212.07.6009) ---
...
def _traducir_resultados_para_reporte(row, asientos_en_cg_set, df_cg):
    ...
    # 2. VALIDACIÓN CONTABILIDAD GENERAL (CG)
    ...
    # --- A. DEFINICIÓN DE CUENTA ESPERADA ---
    mapa_cuentas = {
        'IVA': '2.1.3.05.1.001', 
        'ISLR': '2.1.3.02.1.002', 
    ...
    cuenta_objetivo = None
    for key, value in mapa_cuentas.items():
        if key in subtipo_cp:
            cuenta_objetivo = value
            break
    
    # --- B. VALIDACIÓN DE CUENTA Y MONTO ---
    if estado_galac == 'Error de Subtipo':
        errores_cg.append('Cuenta Contable no coincide (debido a Error de Subtipo en GALAC)')
    elif cuenta_objetivo:
        # 1. ADN Numérico de la cuenta (Ej: '2.1.3.05.1.001' -> '213051001')
        cuenta_objetivo_clean = re.sub(r'\D', '', cuenta_objetivo)
        
        # 2. ADN Numérico de las cuentas en el asiento
        transacciones_asiento['CUENTA_LIMPIA'] = transacciones_asiento['CUENTACONTABLE'].astype(str).apply(lambda x: re.sub(r'\D', '', x))
        
        # 3. Filtrar líneas que coinciden
        lineas_cuenta = transacciones_asiento[transacciones_asiento['CUENTA_LIMPIA'] == cuenta_objetivo_clean]
        
        if lineas_cuenta.empty:
            # Si no hay match, informamos qué cuenta se buscó pero no se halló
            errores_cg.append(f'Cuenta {cuenta_objetivo} no hallada en este asiento del diario')
        else:
            monto_cp = row.get('Monto', 0)
            
            # 4. Decidir columna (Normalmente Retención es CRÉDITO, salvo en Notas de Crédito)
            aplicacion_text = str(row.get('Aplicacion', '')).upper()
            es_nota_credito = any(x in aplicacion_text for x in ['NC', 'NOTA CREDITO'])
            
            # En Facturas (Normal), la retención va al CRÉDITO. En NC va al DÉBITO.
            columna_monto_cg = 'DEBITO_NORM' if es_nota_credito else 'CREDITO_NORM'
            ...
            if columna_monto_cg in lineas_cuenta.columns:
                ...
                if not np.isclose(float(monto_cp), float(suma_cg), atol=0.01):
                    errores_cg.append(f'Monto en Cuenta {cuenta_objetivo} no coincide (CP: {monto_cp:,.2f}, CG: {suma_cg:,.2f})')
            else:
                errores_cg.append(f'Columna de monto ({columna_monto_cg}) no hallada en CG')

    # 3. RESULTADO FINAL
    resultado_final_cg = 'Conciliado en CG' if not errores_cg else ' | '.join(errores_cg)
...
def preparar_datos_softland_debito(df_diario, df_mayor, tag_casa):
    """
    Consolida Diario y Mayor agregando metadatos. 
    Mantiene la integridad de los ceros a la izquierda en NIT y Documentos.
    """
    # 1. Unificar fuentes de Softland
    ...
    # 2. Radar de Columnas: Identificar nombres originales del Excel
    col_deb, col_cre, col_rif, col_ref, col_fue, col_nom = [None]*6
    ...
    # --- SUB-FUNCIÓN 1: LIMPIEZA DE NIT PRESERVANDO CEROS ---
    def limpiar_nit_preservando_ceros(val):
        if pd.isna(val) or str(val).strip() == "": return "0"
        # Quitamos decimales si Excel lo leyó como número (ej: 471010.0 -> 471010)
        s_val = str(val).split('.')[0]
        # Dejamos solo los dígitos, manteniendo la cadena de texto con sus ceros
        solo_numeros = re.sub(r'[^0-9]', '', s_val)
        return solo_numeros if solo_numeros else "0"

    # --- SUB-FUNCIÓN 2: EXTRACCIÓN DE DOCUMENTO PRESERVANDO CEROS ---
    def extraer_doc_softland(row):
        # Intentamos primero con Fuente, luego con Referencia
        for campo in [col_fue, col_ref]:
            texto = row.get(campo, "")
            if pd.isna(texto) or str(texto).strip() == "": continue
            
            # Limpiamos símbolos comunes que Softland añade al número
            t_clean = str(texto).replace('#', '').replace('-', '').strip()
            nums = re.findall(r'\d+', t_clean)
            if nums:
                # Retornamos el último bloque numérico como texto para mantener ceros
                return str(nums[-1])
        return ""
    ...
    # Aplicamos la limpieza técnica de NIT solicitada
    df_soft['_NIT_Norm'] = df_soft[col_rif].apply(limpiar_nit_preservando_ceros) if col_rif else "0"
    
    df_soft['_Nombre_Soft'] = df_soft[col_nom].fillna("SIN NOMBRE") if col_nom else "NOMBRE NO DETECTADO"
    
    # 4. Cálculo de montos de IVA
    ...
    # Usamos valor absoluto del neto porque buscamos la magnitud del impuesto
...
def run_conciliation_debito_fiscal(df_soft_total, df_imprenta_logica, tolerancia_bs, log_messages, nombre_a_obviar="FEBECA"):
    """
    Realiza el cruce N-a-N entre Softland e Imprenta.
    Mantiene integridad de ceros a la izquierda y aplica exclusión dinámica.
    """
    log_messages.append(f"\n--- INICIANDO AUDITORÍA DE DÉBITO FISCAL (Excluyendo: {nombre_a_obviar}) ---")
    
    df_imp = df_imprenta_logica.copy()
    
    # --- 1. RADAR DE COLUMNAS PARA IMPRENTA ---
    ...
    col_nom_imp = find_col(['NOMBRE O RAZON SOCIAL', 'NOMBRE', 'RAZON SOCIAL', 'CLIENTE'], df_imp)

    # --- 2. SUB-FUNCIONES DE LIMPIEZA TÉCNICA ---
    def limpiar_nit_preservando_ceros(val):
        if pd.isna(val) or str(val).strip() == "": return "0"
        # Quitamos decimales de Excel (.0)
        s_val = str(val).split('.')[0]
        # Solo dígitos manteniendo la cadena de texto (preserva ceros)
        solo_numeros = re.sub(r'[^0-9]', '', s_val)
        return solo_numeros if solo_numeros else "0"

    def identificar_tipo_y_doc_imp(row):
        # Prioridad: N/C, luego N/D, luego Factura
        ...
        if val_f == "" or val_f.lower() == "nan": return "", "RESUMEN"
        return normalizar_doc_fiscal(val_f), "FACTURA"

    # --- 3. PREPARACIÓN DE DATA IMPRENTA ---
    # Filtro Anti-Totales y filas en verde del libro
    if col_rif:
        df_imp = df_imp[df_imp[col_rif].notna()]
        mask_totales = df_imp.astype(str).apply(lambda x: x.str.contains('TOTALES|RESUMEN', case=False, na=False)).any(axis=1)
        df_imp = df_imp[~mask_totales]

    # Aplicar identificación de Doc y Tipo (Preservando ceros)
    df_imp[['_Doc_Norm', '_Tipo']] = df_imp.apply(identificar_tipo_y_doc_imp, axis=1, result_type='expand')
    df_imp = df_imp[df_imp['_Doc_Norm'] != ""]
    
    # Limpieza de NIT (Preservando ceros)
    df_imp['_NIT_Norm'] = df_imp[col_rif].apply(limpiar_nit_preservando_ceros) if col_rif else "0"
    ...
    # FILTRO DE EXCLUSIÓN DINÁMICO (Imprenta)
    df_imp = df_imp[~df_imp['_Nombre_Imp'].str.upper().str.contains(nombre_a_obviar.upper(), na=False)]

    # --- 4. PREPARACIÓN DE DATA SOFTLAND (AGREGACIÓN) ---
    # Excluimos de Softland antes de agrupar
    soft_total_fil = df_soft_total[~df_soft_total['_Nombre_Soft'].str.upper().str.contains(nombre_a_obviar.upper(), na=False)].copy()

    # Agrupamos Softland porque un documento puede tener varias líneas contables
    soft_grouped = soft_total_fil.groupby(['_NIT_Norm', '_Doc_Norm', 'CASA', '_Tipo'], as_index=False).agg({
        '_Monto_Bs_Soft': 'sum',
        '_Nombre_Soft': 'first'
    })

    # --- 5. CRUCE MAESTRO (OUTER JOIN) ---
    merged = pd.merge(
        soft_grouped, 
    ...
    # --- 6. LÓGICA DE CLASIFICACIÓN DE ESTADO ---
    def clasificar_resultado(row):
        ...
        # 1. Ambos en cero (Filtro de exentos)
        if m_i <= 0.001 and m_s <= 0.001: return "OK", tipo_final, nombre_final
        
        # 2. Solo en Softland
        if row['_merge'] == 'left_only': 
            return "NO APARECE EN LIBRO DE VENTAS", tipo_final, nombre_final
        
        # 3. Solo en Imprenta
        if row['_merge'] == 'right_only': 
            return "NO APARECE EN CONTABILIDAD", tipo_final, nombre_final
        
        # 4. En ambos pero con diferencia
        dif = abs(m_s - m_i)
        if dif > tolerancia_bs: 
            return f"DIFERENCIA DE MONTO (Bs. {dif:,.2f})", tipo_final, nombre_final
        
        return "OK", tipo_final, nombre_final

    # Aplicar Veredicto
    merged[['Estado', '_Tipo_Final', '_Nombre_Final']] = merged.apply(clasificar_resultado, axis=1, result_type='expand')
    
    log_messages.append(f"✔️ Auditoría finalizada con éxito.")
    return merged
...
def _get_base_classification(cuentas_del_asiento, referencia_completa, fuente_completa, referencia_limpia_palabras, monto_suma, monto_max_abs, is_reverso_check=False):
    ...
    # Detecta el proceso de Cruce de Cuentas / Facturas (NUEVA REGLA)
    if "CRUCE" in referencia_completa and ("CUENTA" in referencia_completa or "FACT" in referencia_completa):
        return "Grupo 16: Cuentas por Cobrar - Varios en ME"
...
def _validar_asiento(asiento_group):
   ...
   # GRUPO 16: CXC VARIOS ME
        ...
        texto_asiento = " ".join(asiento_group['Referencia'].astype(str).str.upper())
        cuentas_usadas = set(asiento_group['Cuenta Contable Norm'])
        
        # CASO A: Es un proceso de CARGO A TRANSPORTE
        if "CARGO" in texto_asiento and "TRANSPORTE" in texto_asiento:
            cuenta_errada = normalize_account('1.1.4.01.6.005')
            if cuenta_errada in cuentas_usadas:
                return "Incidencia: CUENTA ERRADA. Se usó 1.1.4.01.6.005 (Cias. Comerc). La correcta es 1.1.4.01.7.044 (Varios ME)."
        
        # CASO B: Es un proceso de CRUCE DE CUENTAS / FACTURAS
        if "CRUCE" in texto_asiento:
            # Verificamos que use la cuenta correcta de Varios ME
            if normalize_account('1.1.4.01.7.044') in cuentas_usadas:
                return "Conciliado"
            else:
                return "Incidencia: Cruce de cuentas sin cuenta 1.1.4.01.7.044."
...
def leer_monto_haberes_pdf(file_pdf):
    ...
    try:
        ...
        with pdfplumber.open(file_pdf) as pdf:
            ...
            # Buscar el patrón "Total de Saldos Negativos:" seguido de un monto
            # Formato esperado: -14.355,40 o 14,355.40
            match = re.search(r"Total de Saldos Negativos:\s*(-?[\d\.,]+)", text)
            if match:
                monto_str = match.group(1).replace('.', '').replace(',', '.')
                # Manejar casos donde el punto era decimal (formato US)
                if monto_str.count('.') > 1: # Formato mil.mil,dec
                     parts = match.group(1).replace('.', '').replace(',', '.')
                     return abs(float(parts))
                return abs(float(monto_str))
    except Exception as e:
        print(f"Error Haberes PDF: {e}")
    return 0.0
...
def procesar_ajustes_balance_usd(f_cb, f_cg, f_hab_usd, f_hab_ves, tasa_bcv, tasa_corp, empresa, n_asiento, df_manual, log):
    log.append(f"--- Iniciando Procesamiento Bimonetario: {empresa} ---")
    
    # Conservar el Balance original para los encabezados de la Hoja 1
    f_cg.seek(0)
    df_balance_raw = pd.read_excel(f_cg, header=None)
    
    # Datos del balance para naturaleza contraria
    ...
    for _, row in df_tesoreria.iterrows():
        ...
        # Valores base del reporte
        s_libros_rep = float(row.get('SALDO EN LIBROS', 0))
        s_bancos_rep = float(row.get('SALDO EN BANCOS', 0))
        mov_no_conc = float(row.get('MOVIMIENTOS EN BANCOS NO CONCILIADOS', 0))

        # --- CÁLCULOS BIMONETARIOS ---
        # a, b: Saldo en Bs
        sl_bs = s_libros_rep if es_local else s_libros_rep * tasa_bcv
        sb_bs = s_bancos_rep if es_local else s_bancos_rep * tasa_bcv
        # c, d: Saldo en $
        sl_usd = s_libros_rep / tasa_corp if es_local else s_libros_rep
        sb_usd = s_bancos_rep / tasa_corp if es_local else s_bancos_rep
        # e, f: Ajustes
        adj_bs = mov_no_conc if es_local else mov_no_conc * tasa_bcv
        adj_usd = mov_no_conc / tasa_corp if es_local else mov_no_conc
...
def procesar_ajustes_balance_usd(f_cb, f_cg, f_hab_usd, f_hab_ves, tasa_bcv, tasa_corp, empresa, n_asiento, df_manual, log):
    ...
    # --- PASO 1: EXTRAER ACTIVO Y PASIVO DEL BALANCE (COLUMNA A) ---
    # Extrae solo cuentas de detalle (que no terminan en 000)
    datos_cg = extraer_saldos_cg_ajustes(f_cg, log) 
    # Filtro adicional solicitado: Cuentas detalle 1.x y 2.x
    cuentas_detalle = {k: v for k, v in datos_cg.items() 
                       if (k.startswith('1') or k.startswith('2')) and not k.endswith('.000')}

    asientos = []
    resumen_ajustes = [] # Datos para Hoja 1
    detalles_bancos = [] # Datos para Hoja 2

    # --- PASO 2: AJUSTE DE BANCOS (LÓGICA L/E) ---
    df_tesoreria = pd.read_excel(f_cb, header=7) # Asumiendo encabezado en fila 8
    df_tesoreria.columns = [str(c).upper().strip() for c in df_tesoreria.columns]
    
    sum_ajustes_bancos_usd = 0.0

    for idx, row in df_tesoreria.iterrows():
        cta_c = str(row.get('CUENTA CONTABLE', '')).strip()
        if not cta_c or cta_c == 'nan' or not cta_c.startswith('1.'): continue
        
        # Identificar Moneda (L o E)
        cta_bancaria_str = str(row.get('CUENTA BANCARIA', ''))
        tipo_moneda = 'L' if 'L' in cta_bancaria_str else 'E'
        
        # Valores de tesorería
        s_libros_orig = float(row.get('SALDO EN LIBROS', 0))
        s_bancos_orig = float(row.get('SALDO EN BANCOS', 0))
        mov_no_conc_orig = float(row.get('MOVIMIENTOS EN BANCOS NO CONCILIADOS', 0))

        # Cálculos Paso 2.1
        # a, b: Saldo Libros y Bancos BS
        s_libros_bs = s_libros_orig if tipo_moneda == 'L' else s_libros_orig * tasa_bcv
        s_bancos_bs = s_bancos_orig if tipo_moneda == 'L' else s_bancos_orig * tasa_bcv
        
        # c, d: Saldo Libros y Bancos $
        s_libros_usd = s_libros_orig / tasa_corp if tipo_moneda == 'L' else s_libros_orig
        s_bancos_usd = s_bancos_orig / tasa_corp if tipo_moneda == 'L' else s_bancos_orig
        
        # e, f: AJUSTE BS y $
        ajuste_bs = mov_no_conc_orig if tipo_moneda == 'L' else mov_no_conc_orig * tasa_bcv
        ajuste_usd = mov_no_conc_orig / tasa_corp if tipo_moneda == 'L' else mov_no_conc_orig
        
        tasa_calc = ajuste_bs / ajuste_usd if ajuste_usd != 0 else 0
        verificacion = s_bancos_orig - s_libros_orig - mov_no_conc_orig

        # Guardar para Hoja 2
        detalles_bancos.append({
            **row.to_dict(),
            'SALDO EN LIBROS BS': s_libros_bs,
            'SALDO EN BANCOS BS': s_bancos_bs,
            'SALDO EN LIBROS $': s_libros_usd,
            'SALDO EN BANCOS $': s_bancos_usd,
            'AJUSTE BS': ajuste_bs,
            'AJUSTE $': ajuste_usd,
            'TASA_CALC': tasa_calc,
            'VERIFICACION': verificacion
        ...
        # Paso 2.2: Acumular para Hoja 1 y Asiento
        if abs(ajuste_usd) > 0.001:
            resumen_ajustes.append({'Cuenta': cta_c, 'Origen': 'Bancos', 'Ajuste USD': ajuste_usd})
            sum_ajustes_bancos_usd += ajuste_usd
            # Registro en Asiento: Banco vs Deudores
            asientos.append({'Cuenta': cta_c, 'Desc': row.get('DESCRIPCIÓN'), 'DebeUSD': ajuste_usd if ajuste_usd > 0 else 0, 'HaberUSD': abs(ajuste_usd) if ajuste_usd < 0 else 0})

    # Paso 2.3: Contrapartida de bancos (Deudores)
    if abs(sum_ajustes_bancos_usd) > 0.001:
        asientos.append({
            'Cuenta': '1.1.3.01.1.001', 
            'Desc': 'Contrapartida Ajuste Bancos', 
    ...
    # --- PASO 3: AJUSTE HABERES CLIENTES ---
    m_hab_ves = leer_monto_haberes_pdf(f_hab_ves)
    m_hab_usd = leer_monto_haberes_pdf(f_hab_usd)
    ...
    # --- PASO 4: NATURALEZA CONTRARIA ---
    for cta, data in cuentas_detalle.items():
        s_usd = data['USD']
        if s_usd < -0.01: # Si es negativo (Naturaleza contraria)
            monto_adj = abs(s_usd)
            contrapartida = MAPEO_SALDOS_CONTRARIOS.get(cta, '1.1.3.01.1.001') # Default a Deudores
            asientos.append({'Cuenta': cta, 'Desc': data['descripcion'], 'DebeUSD': monto_adj, 'HaberUSD': 0})
            asientos.append({'Cuenta': contrapartida, 'Desc': 'Reclasificacion Naturaleza', 'DebeUSD': 0, 'HaberUSD': monto_adj})
            resumen_ajustes.append({'Cuenta': cta, 'Origen': 'Naturaleza', 'Ajuste USD': monto_adj})

    # --- PASO EXTRA: AJUSTES MANUALES ---
    for _, adj in df_manual.iterrows():
        m_usd = adj['Monto en USD']
        t_tipo = adj['Tasa de conversión']
        t_valor = tasa_bcv if t_tipo == "BCV" else tasa_corp
        asientos.append({'Cuenta': adj['Cuenta a ajustar'], 'Desc': 'Ajuste Manual', 'DebeUSD': m_usd, 'HaberUSD': 0})
        asientos.append({'Cuenta': adj['Cuenta contrapartida'], 'Desc': 'Contrapartida Manual', 'DebeUSD': 0, 'HaberUSD': m_usd})
        resumen_ajustes.append({'Cuenta': adj['Cuenta a ajustar'], 'Origen': 'Manual', 'Ajuste USD': m_usd})

    # Preparación final de DataFrames para el reporte
    df_asiento = pd.DataFrame(asientos)
    if not df_asiento.empty:
        # Se asume tasa BCV para la valoración del cargador en Bs.
        df_asiento['Débito VES'] = (df_asiento['DebeUSD'] * tasa_bcv).round(2)
        df_asiento['Crédito VES'] = (df_asiento['HaberUSD'] * tasa_bcv).round(2)

    return pd.DataFrame(resumen_ajustes), pd.DataFrame(detalles_bancos), df_asiento, None, {'tasa_bcv': tasa_bcv}
