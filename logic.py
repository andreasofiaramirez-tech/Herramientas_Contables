...
def run_conciliation_comisiones_bancarias(df_cb_raw, df_cg_raw, empresa_sel, log_messages):
    ...
    col_deb_ves = next((c for c in df_cg_raw.columns if any(k in normalizar_texto_busqueda(c) for k in ['DEBITO', 'DEBE']) and any(k in normalizar_texto_busqueda(c) for k in ['VES', 'LOCAL', 'BOLIVAR'])), 'Débito VES')
    col_cre_ves = next((c for c in df_cg_raw.columns if any(k in normalizar_texto_busqueda(c) for k in ['CREDITO', 'HABER']) and any(k in normalizar_texto_busqueda(c) for k in ['VES', 'LOCAL', 'BOLIVAR'])), 'Crédito VES')
...
def run_conciliation_anexos(df_cb_raw, df_cg_raw, empresa_sel, log_messages):
    ...
    # 2. PROCESAMIENTO DE TESORERÍA (CB)
    header_idx = None
